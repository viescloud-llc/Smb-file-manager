...
public class FileMetaDataService extends ViesService<FileMetaData, Integer, FileMetaDataDao> {
    ...
    public FileMetaDataService(DatabaseUtils<FileMetaData, Integer> databaseUtils, FileMetaDataDao repositoryDao, SmbSessionFactory smbSessionFactory) {
    ...
    public boolean isFileExist(String path) {
        try {
            ...
            if(exist) {
                ...
                var metadata = FileMetaData.builder().contentType(contentType).originalFilename(fileName).path(path).size(size).userIds(List.of(userId)).build();
